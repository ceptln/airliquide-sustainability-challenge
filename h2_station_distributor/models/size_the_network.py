...
def check_parameters_quality(parameters: dict):
    ...
    if not ('small' in parameters['prefered_order_of_station_type'] \
        ...
        raise ValueError(
            "The prefered_order_of_station_type must contain 'small', 'medium' and 'large' and nothing else")
...
def compute_quantity_h2(parameters: dict, manufacturers_desc: dict, verbose: bool = True) -> dict[int, dict]:
    ...
    actual_autonomy_man_1 = round(parameters['manufacturers_desc']['man_1']['autonomy'] * (
                1 - parameters['average_tank_filling_rate_before_refill']))
    actual_autonomy_man_2 = round(parameters['manufacturers_desc']['man_2']['autonomy'] * (
                1 - parameters['average_tank_filling_rate_before_refill']))
    actual_autonomy_man_3 = round(parameters['manufacturers_desc']['man_3']['autonomy'] * (
                1 - parameters['average_tank_filling_rate_before_refill']))
    ...
    if verbose:
        ...
        print(
            f"  - Man.1 ({manufacturers_desc['man_1']['name']}): {nb_daily_trucks_man_1} ({round(parameters['split_manufacturer']['man_1'], 2) * 100}%)")
        print(
            f"  - Man.2 ({manufacturers_desc['man_2']['name']}): {nb_daily_trucks_man_2} ({round(parameters['split_manufacturer']['man_2'], 2) * 100}%)")
        print(
            f"  - Man.3 ({manufacturers_desc['man_3']['name']}): {nb_daily_trucks_man_3} ({round(parameters['split_manufacturer']['man_3'], 2) * 100}%)")
...
def compute_number_of_stations(quantity_h2: int, parameters: dict, stations_desc: dict) -> int:
    ...
    return int(round(quantity_h2 / (parameters['split_station_type']['small'] * stations_desc['small']['storage_onsite'] \
                                    + parameters['split_station_type']['medium'] * stations_desc['medium'][
                                        'storage_onsite'] \
                                    + parameters['split_station_type']['large'] * stations_desc['large'][
                                        'storage_onsite']) + 0.5))
...
def perform_best_stations_split(quantity_h2: int, parameters: dict, stations_desc: dict, verbose: bool = True) -> dict:
    ...
    # may have to delete the smallest stations several times without being able to delete a medium one (depends on the
    # stations config)
    for _ in range(5):
...
def define_national_strategies(quantity_h2: int, parameters: dict, stations_desc: dict,
                               verbose: bool = False) -> pd.DataFrame:
    """ This function... """
    national_strategy = perform_best_stations_split(quantity_h2, parameters=parameters, stations_desc=stations_desc,
                                                    verbose=verbose)
    national_strategy['region'] = 'Total'
    return pd.DataFrame(national_strategy, index=[0])[
        ['region', 'quantity_h2_to_reach', 'quantity_h2_proposed', 'small', 'medium', 'large', 'total']]


def define_regional_strategies(quantity_h2: int, region_breakdown: dict, parameters: dict, stations_desc: dict,
                               verbose: bool = False) -> pd.DataFrame:
    """ This function... """
    regional_strategies_list = []
    for region, prop in region_breakdown.items():
        regional_quantity_h2 = quantity_h2 * prop
        regional_strategy = perform_best_stations_split(regional_quantity_h2, parameters=parameters,
                                                        stations_desc=stations_desc, verbose=verbose)
        regional_strategy['region'] = region
        regional_strategies_list.append(regional_strategy)
    regional_strategies = pd.DataFrame(regional_strategies_list)[
        ['region', 'quantity_h2_to_reach', 'quantity_h2_proposed', 'small', 'medium', 'large', 'total']]
    return regional_strategies.append(regional_strategies.sum(numeric_only=True), ignore_index=True).fillna('Total')


def define_best_regional_strategies(parameters: dict, manufacturers_desc: dict, stations_desc: dict,
                                    region_breakdown: dict, verbose: bool = False) -> pd.DataFrame:
    """ This function... """
...
def define_best_national_strategy(parameters: dict, manufacturers_desc: dict, stations_desc: dict,
                                  verbose: bool = False) -> pd.DataFrame:
    """ This function... """
    quantity_h2 = compute_quantity_h2(parameters=parameters, manufacturers_desc=manufacturers_desc, verbose=verbose)
    return define_national_strategies(quantity_h2=quantity_h2, parameters=parameters, stations_desc=stations_desc,
                                      verbose=verbose)
